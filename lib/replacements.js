var replacements = [
        { // replace the }(window.jQuery) at the bottom with ender
          ...
          , replace: '}(require(\'ender-bootstrap-base-faker\'))'
        }
      ...
      , { // affix uses .is(':visible')
            packageMatch: /affix/
          , regex: /this\.\$element\.is\(\':visible\'\)/
          , replace: '(this.$element.get(0).offsetWidth !== 0 && this.$element.get(0).offsetHeight !== 0)'
        }
      , {
            packageMatch: /typeahead/
          , regex: /\$\.isFunction\(([^)]+)\)/
      ...
      , { // collapse uses $.Event('foo') and fetches the type with event.type, squash this
            packageMatch: /collapse/
          , regex: /startEvent\.type == 'show'/
          , replace: 'startEvent == "show"'
        }
      , { // typeahead and tooltip use `currentTarget` where it may not be available in old IE
            packageMatch: /typeahead|tooltip/
          , regex: /e\.currentTarget/g
          , replace: '(e.currentTarget || e.target)'
        }
